...
namespace mset
{
	public class SkyDebug : MonoBehaviour
	{
		...
		public string GetDebugString()
		{
			...
			material = ((!Application.isPlaying) ? base.renderer.sharedMaterial : base.renderer.material);
			...
			block.Clear();
			base.renderer.GetPropertyBlock(block);
			for (int i = 0; i < 2; i++)
			{
		...
		{
			bool flag = printOnce || printConstantly;
			if ((bool)base.renderer && printInEditor && printToConsole && flag)
			{
				printOnce = false;
