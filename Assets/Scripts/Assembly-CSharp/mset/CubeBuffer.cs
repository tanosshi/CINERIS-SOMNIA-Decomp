	...
	public class CubeBuffer
	{
		...
		private static void linkEdges()
		{
			if (_leftEdges == null)
			{
				...
				_leftEdges[1] = new CubeEdge(5, false, false);
				_leftEdges[0] = new CubeEdge(4, false, false);
				_leftEdges[3] = new CubeEdge(1, true, true);
				_leftEdges[2] = new CubeEdge(1, false, true, true);
				_leftEdges[5] = new CubeEdge(0, false, false);
				_leftEdges[4] = new CubeEdge(1, false, false);
				_rightEdges = new CubeEdge[6];
				_rightEdges[1] = new CubeEdge(4, false, false);
				_rightEdges[0] = new CubeEdge(5, false, false);
				_rightEdges[3] = new CubeEdge(0, false, true, true);
				_rightEdges[2] = new CubeEdge(0, true, true);
				_rightEdges[5] = new CubeEdge(1, false, false);
				_rightEdges[4] = new CubeEdge(0, false, false);
				_upEdges = new CubeEdge[6];
				_upEdges[1] = new CubeEdge(2, false, true, true);
				_upEdges[0] = new CubeEdge(2, true, true);
				_upEdges[3] = new CubeEdge(4, false, false);
				_upEdges[2] = new CubeEdge(5, true, false, true);
				_upEdges[5] = new CubeEdge(2, true, false, true);
				_upEdges[4] = new CubeEdge(2, false, false);
				_downEdges = new CubeEdge[6];
				_downEdges[1] = new CubeEdge(3, true, true);
				_downEdges[0] = new CubeEdge(3, false, true, true);
				_downEdges[3] = new CubeEdge(5, true, false, true);
				_downEdges[2] = new CubeEdge(4, false, false);
				_downEdges[5] = new CubeEdge(3, true, false, true);
				_downEdges[4] = new CubeEdge(3, false, false);
				for (int i = 0; i < 6; i++)
				{
		...
		public void sampleBicubic(ref Color dst, float u, float v, int face)
		{
			...
			for (int k = 0; k < 4; k++)
			{
				...
				cubicKernel[0, k] = Color.Lerp(a, color3, u);
			}
			b = cubicKernel[0, 0];
		...
		public void resampleToCube(ref Cubemap cube, int mip, ColorMode cubeColorMode, bool useGamma, float exposureMult)
		{
			...
			for (int i = 0; i < 6; i++)
			{
				resampleFace(ref dst, i, num, false);
				if (exposureMult != 1f)
				{
		...
		public void resampleToBuffer(ref CubeBuffer dst, float exposureMult)
		{
			...
			for (int i = 0; i < 6; i++)
			{
				resampleFace(ref dst.pixels, i * num, i, dst.faceSize, false);
		...
		public void fromHorizCrossTexture(Texture2D tex, int mip, ColorMode texColorMode, bool useGamma)
		{
			...
			for (int i = 0; i < 6; i++)
			{
				...
				pixelCopyBlock(ref dst2, 0, 0, faceSize, dst, src_x, src_y, faceSize * 4, faceSize, faceSize, true);
