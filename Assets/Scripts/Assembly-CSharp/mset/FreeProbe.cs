...
namespace mset
{
	...
	public class FreeProbe : MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			{
				base.camera.CopyFrom(skyManager.ProbeCamera);
			}
			else if ((bool)Camera.main)
			{
				base.camera.CopyFrom(Camera.main);
			}
		}
		...
		public void RunQueue()
		{
			...
			{
				base.camera.CopyFrom(Camera.main);
				defaultCullMask = base.camera.cullingMask;
			}
			...
			base.camera.fieldOfView = 90f;
			base.camera.clearFlags = CameraClearFlags.Skybox;
			base.camera.cullingMask = defaultCullMask;
		...
		private void OnPreCull()
		{
			if (stage == Stage.CAPTURE || stage == Stage.CONVOLVE || stage == Stage.PRECAPTURE)
			{
				...
				base.camera.rect = new Rect(0f, 0f, width, height);
				base.camera.pixelRect = new Rect(0f, 0f, num, num);
		...
		private void Update()
		{
			...
			if ((bool)RT && captureHDR && stage == Stage.CAPTURE)
			{
				...
				bool hdr = base.camera.hdr;
				base.camera.hdr = true;
