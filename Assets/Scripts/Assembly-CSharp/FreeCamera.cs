...
public class FreeCamera : MonoBehaviour
{
	...
	public void Start()
	{
		...
		{
			usePivotPoint = true;
			Vector3 v = base.transform.worldToLocalMatrix.MultiplyPoint3x4(pivotTransform.position);
			v.x = 0f;
			v.y = 0f;
			targetDist = v.z;
			target.position = base.transform.localToWorldMatrix.MultiplyPoint3x4(v);
		}
		else
		{
	...
	public void Update()
	{
		inputBounds.x = base.camera.pixelWidth * paramInputBounds.x;
		inputBounds.y = base.camera.pixelHeight * paramInputBounds.y;
		inputBounds.width = base.camera.pixelWidth * paramInputBounds.width;
		inputBounds.height = base.camera.pixelHeight * paramInputBounds.height;
