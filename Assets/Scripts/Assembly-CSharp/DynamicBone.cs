	...
	private void ApplyParticlesToTransforms()
	{
		...
		for (int i = 1; i < m_Particles.Count; i++)
		{
			...
			if (particle2.m_Transform.childCount <= 1)
			{
				...
				if (flag)
				{
					vector = MirrorVector(vector, right);
				}
				if (flag2)
				{
					vector = MirrorVector(vector, up);
				}
				if (flag3)
				{
					vector = MirrorVector(vector, forward);
				}
				Quaternion quaternion = Quaternion.FromToRotation(particle2.m_Transform.TransformDirection(direction), vector);
